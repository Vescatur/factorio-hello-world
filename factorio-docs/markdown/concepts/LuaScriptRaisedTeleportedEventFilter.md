...
| Name | Type | Optional |
| --- | --- | --- |
| [filter](#filter) | `"ghost"` ∣ `"rail"` ∣ `"rail-signal"` ∣ `"rolling-stock"` ∣ `"robot-with-logistics-interface"` ∣ `"vehicle"` ∣ `"turret"` ∣ `"crafting-machine"` ∣ `"wall-connectable"` ∣ `"transport-belt-connectable"` ∣ `"circuit-network-connectable"` ∣ `"type"` ∣ `"name"` ∣ `"ghost_type"` ∣ `"ghost_name"` |  |
