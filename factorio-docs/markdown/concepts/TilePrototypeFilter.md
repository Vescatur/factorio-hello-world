...
| Name | Type | Optional |
| --- | --- | --- |
| [filter](#filter) | `"minable"` ∣ `"autoplace"` ∣ `"blueprintable"` ∣ `"item-to-place"` ∣ `"collision-mask"` ∣ `"walking-speed-modifier"` ∣ `"vehicle-friction-modifier"` ∣ `"decorative-removal-probability"` ∣ `"absorptions-per-second"` |  |
